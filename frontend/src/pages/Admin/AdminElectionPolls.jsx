...
const VotesPanel = ({ candidate, election, ward, onClose }) => {
  ...
  useEffect(() => {
    const fetchVoters = async () => {
      setLoading(true);
      try {
        const res = await voteService.getElectionResultsForAdmin(election.id, ward.id);
...
const AdminElectionPolls = () => {
  ...
  const fetchAllElections = async () => {
    try {
      const res = await electionService.getCompletedElectionsForAdmin();
      if (res.success) {
...
export default AdminElectionPolls;