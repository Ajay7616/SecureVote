...
/* ─── Main Component ─────────────────────────────────────────────────────── */
const MasterAdminCandidatesList = () => {
  ...
  const handleWardClick = async (ward) => {
    ...
    try {
      ...
      const res = await candidateService.getAdminCandidateByElectionAndWard(selectedElection.id, ward.id);
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (selectedWard) {
        const res = await candidateService.getAdminCandidateByElectionAndWard(selectedElection.id, selectedWard.id);
        if (res.success) setWardCandidates(res.data || []);
      }
    } catch (err) {
...
export default MasterAdminCandidatesList;