...
const MasterAdminVotersList = () => {
  ...
  const handleWardClick = async (ward) => {
    ...
    try {
      const res = await voterService.getAllVoters(ward.id, ward.election_id);
      setVoters(res.data || []);
    ...
    try {
      ...
      const results = await Promise.all(
        electionWardList.map(w =>
          voterService.getAllVoters(w.id, election.id).then(r => ({ voters: r.data || [], ward: w }))
        )
      );
      const wb = XLSX.utils.book_new();
      let totalVoters = 0;
...
export default MasterAdminVotersList;